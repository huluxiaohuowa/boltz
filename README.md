...
`boltz-host-metrics` runner 默认每 5 秒写入这个文件。Web 容器不需要 NVIDIA runtime；它只读取 JSON。runner 会在宿主机 namespace 中采集 GPU：Jetson/Thor/Tegra 主机优先使用 `tegrastats` 或 Jetson sysfs，不走 `nvidia-smi`；x86 NVIDIA 使用 `nvidia-smi`；ROCm 使用 `rocm-smi`。没有可用 GPU 工具时，页面会明确显示 GPU 不可见，而不是伪造占用数据。
