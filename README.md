...
Boltz WebApp 是面向 CADD 研究人员的一体化分子设计工作台。它把靶点蛋白导入与清理、共晶配体识别与提取、口袋定义、配体库构建与 2D 编辑、配体标准化、任务队列、结果文件管理和 Boltz 输入生成集中到同一个浏览器界面里。研究人员可以从 PDB ID、本地结构文件、SDF、SMILES、表格或手绘分子出发，把每一步生成的蛋白、口袋、配体、配置和结果都沉淀为可下载、可复用、可跨项目复制、可被 API 自动化调用的资产。

这套组件的目标不是只提供一个模型入口，而是覆盖早期结构药物设计中“导入结构 → 检查共晶信息 → 整理蛋白 → 选择/提取配体 → 定义口袋 → 准备配体 → 生成 Boltz 输入 → 跟踪任务 → 复用输出”的完整准备链路。它适合用于靶点评估、共晶结构整理、分子对接输入准备、Boltz 亲和力/复合物预测输入准备，以及后续分子生成、FEP、SAR 和 TPD/PROTAC 工作流的资产底座。
...
- 系统资源：通过 host metrics runner 把宿主机 CPU、逻辑线程、内存、磁盘和 GPU 采样写入共享目录，Web 页面准实时展示。
...
`boltz-host-metrics` runner 默认每 5 秒写入这个文件。Web 容器不需要 NVIDIA runtime；它只读取 JSON。runner 会在宿主机 namespace 中尝试调用 `nvidia-smi`、`rocm-smi` 或 `tegrastats`，适配 AMD/x86 NVIDIA、ROCm 和 Jetson/Thor 场景。没有可用 GPU 工具时，页面会明确显示 GPU 不可见，而不是伪造占用数据。
...
docker compose --env-file .env.web \
  ...
  -f docker-compose.host-metrics.yml \
  -f docker-compose.protein-prep.arm.yml \
  -f docker-compose.ligand-prep.arm.yml \
  --profile host-metrics --profile protein-prep --profile ligand-prep \
  up -d
```
...
Boltz 模型、原始代码和论文信息见 [README.origin.md](README.origin.md)。本仓库的 WebApp、任务编排、资产管理和部署脚本围绕 CADD 工作流做工程化封装。

## English Guide

Boltz WebApp is an integrated molecular-design workbench for CADD teams. It combines target protein import and cleanup, co-crystal ligand detection and extraction, pocket definition, ligand-library creation and 2D editing, ligand standardization, task queues, result-file management, and Boltz-ready input generation in one browser interface. Researchers can start from a PDB ID, local structure files, SDF, SMILES, tables, or hand-drawn molecules, then keep every generated protein, pocket, ligand, configuration, and result as a downloadable, reusable, cross-project-copyable, API-addressable asset.

The suite is designed as a complete preparation chain, not just a model launcher: import structures, inspect co-crystal content, clean proteins, select or extract ligands, define pockets, prepare ligands, generate Boltz inputs, track jobs, and reuse outputs. It is intended for target assessment, co-crystal structure curation, docking input preparation, Boltz affinity/complex-prediction input preparation, and as the asset layer for later molecule generation, FEP, SAR, and TPD/PROTAC workflows.
...
- System resources: a host metrics runner writes host CPU, logical-core, memory, disk, and GPU samples into the shared data directory, and the Web UI displays them near real time.
...
The `boltz-host-metrics` runner writes this file every 5 seconds by default. The Web container does not need NVIDIA runtime; it only reads the JSON snapshot. The runner enters the host namespace and tries `nvidia-smi`, `rocm-smi`, or `tegrastats`, covering AMD/x86 NVIDIA, ROCm, and Jetson/Thor deployments. If no GPU tool is available, the page explicitly reports that GPU data is not visible instead of fabricating utilization values.
